...
/* * need to be modified to suit the decay dark matter */
/* * sigmaV is the total cross section for annihilation DM 
     changed it to the total decay width for decaying DM
     the density profile also needs to be changed to the sqrt root.
*/
// DecayDM is defined in micromegas.h as global parameter
// DecayDM = 0 for annihilation
// DecayDM = 1 for decay
...
double hProfileZhao(double r)
{
   ...
   if(DecayDM)
     { 
       //       printf("Decaying DM\n");
       if(rho >= rhomax) return sqrt(rhomax); else return sqrt(rho);
     }
...
{ 
   if(DecayDM)
     {
       return  exp(-(pow(r/Rsun,alphaEin)-1)/alphaEin);
     }

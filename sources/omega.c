...
{ 
   ...
   if(m1_<0) return 0;
   m1_=sqrt(m1_);
   return  gauss(intDecay2, atan(-m2_p/w2_), atan( ((m0_-m1_)*(m0_-m1_)-m2_p*m2_p)/(m2_p*w2_)),5);
}

static double decayPcmWgauss(double m0,double m1,double m2,double w1,double w2)
{  
  m0_=m0;
  ...
  { if(m1>m0) return 0;
    ...
    m2_p=m2;
    w2_=w2;
    return gauss(intDecay2, atan(-m2/w2), atan( ((m0-m1)*(m0-m1)  -m2*m2)/(m2*w2)),5)/M_PI/ME(m0,m1,m2); 
  }
  else if(w2==0)
  { if(m2>m0) return 0;
    m1_=m2;
    m2_p=m1;
    w2_=w1;
    return gauss(intDecay2, atan(-m1/w1), atan( ((m0-m2)*(m0-m2)-m1*m1)/(m1*w1)),5)/M_PI/ME(m0,m1,m2);
  }
  else 
  { w1_=w1;
    w2_=w2; 
    m1_p=m1;
    m2_p=m2; 
    return gauss(intDecay1, atan(-m1/w1), atan( (m0*m0-m1*m1)/(m1*w1)),5)/(M_PI*M_PI)/ME(m0,m1,m2);
  }
}
...
{ double r; 
  ...
  { int l,l_;
    ...
    { double m1,m2,sqrtS;
      ...
      if(wV1*wV2>0) r*=decayPcmW(sqrtS,MV1,MV2,wV1,wV2)/decayPcmW(sqrtS,MV1,MV2,wV1,0);    
    } 
  }   
  else
  {  if(kin22(PcmIn,pmass)) return 0.; 
     ...
     { double f; 
       ...
       if( sqrtS-pmass[2]-pmass[3] < 15*(AUX[nsub22].w[0]+AUX[nsub22].w[1]))   
         f=decayPcmWgauss(sqrtS,pmass[2],pmass[3],AUX[nsub22].w[0],AUX[nsub22].w[1])/decayPcm(sqrtS,pmass[2],pmass[3]);     
       else   
       {   f=1; 

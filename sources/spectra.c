...
{ double r;
  if(e<=0) return 0;
  r=zInterp(log(e/mD2_),tabStat)/(e*sqrt(e*(e+2*m_)));/* dN/dz=e*dN/de,sqrt(e*(e+2*m_))=m_*shY' */
  return r;
}
...
{ double chY=cosh(Y), shY=sinh(Y);
  ...
  for(l=0;l<NZ;l++)
  { double e=Emax*exp(Zi(l)); /* e is the kinetic energy in the frame where mx is moving, e1,e2 are in mx-rest frame */
    double p=sqrt(e*(e+2*mx));/* p^2 + mx^2 = (e + mx)^2 */ 
    double e1=chY*(e+mx)-shY*p-mx;/* opposite direction, minimal energy */
    double e2=chY*(e+mx)+shY*p-mx;/* same direction, maximal energy */
    if(e2>mD2_) e2=mD2_;
...
static void getSpectrum(int wPol, double M, double m1,double m2,char*n1,char*n2, int N1, int N2,int outP, double *tab)
{
  ...
  if(inP>=0) /* if the final states N1 and N2 are the above particles */
  {
    if(wPol &&(inP==10 ||inP==13 )) /* for Z and W, wPol for polarization */
...
/*        if(PrintOn && outP==0 ) printf(" %.0f%% transverse W produced\n",100*fabs(left+right));*/
  ...
  else  /* else then the particle N1 and N2 may decay to two daughter particles which may decay further */
  { char* nn[2];
    double mm[2];
    double E[2]; 
    double p2;
    ...
    { double tabAux[NZ];
      ...
      {  
         ...
         case 11: case 13: case 15: inP=(N+1)/2+1; break;/* e, mu, tau*/
         case 23:inP=10; break;  /*z*/
         case 24:inP=13; break;  /*w*/ /*the rest are neutrino and gamma */
      ...
      { double w=0;
        ...
        Y=acosh(E[k]/mm[k]);
        boost(Y, M/2, mm[k], outMass[outP], tabAux); /* boost back to the frame in which the parent particle mm[k] is*/
        for(i=0;i<NZ;i++)tab[i]+=tabAux[i]/w;  /* Normalization, divided by decay width */
      }
    } 
  }
}
...
// forbidden Xmu in the final states
  libPtr=getMEcode(0,ForceUG,process,NULL,NULL,lib);
...
#endif 
      if(v_cs[k]<0) v_cs[k]=0; /* why negative values appear? numerical issues? */
      vcsSum+=v_cs[k];
    } else v_cs[k]=-1;
//    printf("vcs=%E cc23=%p \n",v_cs[k],cc23);
  ...
  { char * N[4];
    ...
    if(Spectra[0] && plusA && (charge3[0] || charge3[1])&& (m[2]+m[3]< m[0]+m[1])) 
    {
       ...
       {  double dp=(Mcdm-Eg/2)*sqrt((1-ms*ms/kappa)*(1-md*md/kappa));
          ...
          double  m_min=10*Mcdm;// what does this mean?
          int n,m,w;
          char *s;
/*             
...
/*        printf("%20.20s  d<x^2>=%.1E\n",buff, x2/x2Sum); */
  ...
  /* check whether the dark matter is majarana particle, if not, calculate */
  ...
  {   
    ...
    c=1/(1+cosh(dmAsymm));// If not majarana, times an additonal factor 1/2        
    vcs*=c; 
    for(l=0;l<6;l++) if(Spectra[l])for(i=0;i<NZ;i++)Spectra[l][i]*=c;    
  }      
  
  if(vcs) for(l=0;l<6;l++) if(Spectra[l])for(i=0;i<NZ;i++)Spectra[l][i]/=vcs;
  // *the above line just normalize the individual channel over the total
  // *cross section vcs

...
{  int err;
  ...
  double MVx=10000,MSx=300,MHiggs=125;
  double MSx2,MHiggs2;
  ...
  //   printf("# MVx=%.2E\t MSx=%.2E\n ",MVx,MSx);
   printf("# MVx  \t  MSx \t  gxx   \t  Sin(alpha)  \t   lambdaSH  \t   lambdaS  \t   Xf    \t   Omegah^2  \t  sigam_p \n");
   
   //      switch(Case_i){
   //      case 1:
   // MSx=300; MVx=500; gxx=0.41;mixingA=asin(0.150);// break;
	//      case 2:
   // MSx=300; MVx=250; gxx=1.30;mixingA=asin(0.046);//break;
	//     case 3:
   // MSx=300; MVx=160; gxx=0.12;mixingA=asin(0.104);//break;
	//     case 4:
   // MSx=300; MVx=100; gxx=0.25;mixingA=asin(0.199);//break;

   // MVx=400; MSx=500; gxx=0.34 ; mixingA=asin(0.305);
 
   // MVx=400; MSx=250; gxx=0.37 ; mixingA=asin(0.140);

   // MVx=120; MSx=200; gxx=0.35 ; mixingA=asin(0.171);
      
    MVx=120; MSx=100; gxx=0.22 ; mixingA=asin(0.200);
	//      }
...
{ int fast=1;
  ...
  printf("%.1f\t %.1f\t %.2E\t %.2E \t %.2E \t %.2E \t %.2E \t %.2E \t",MVx,MSx,gxx,SNA,lambdaSH,lambdaS,Xf,Omega);
...
{ 
  ...
  double FluxNe[NZ],FluxNm[NZ],FluxNl[NZ];
...
printf("\n==== Indirect detection =======\n");  
  ...
  char outputFilename[200] = "/home/ytang/test.dat";
  char outputdNdE[200] = "/home/ytang/test.dat";
  sprintf(outputFilename,"/home/ytang/anni_gamma_%.0f_%.0f.dat",Mcdm, MSx);
  sprintf(outputdNdE,"/home/ytang/dNdE_gamma_%.0f_%.0f.dat",Mcdm, MSx);

  FILE *ifp, *ofp, *ofpdNE;
  ofp = fopen(outputFilename, "w");
  ofpdNE=fopen(outputdNdE, "w");
  ...
  { 
	...
	fprintf(ofpdNE,"%d\t %.2E\t %.2E\n",ii,Etest,SpectdNdE(Etest, SpA));
      }

     double fi=0.52,dfi=0.05; /* angle of sight and 1/2 of cone angle in [rad] */ 
	...
	Etest=1.0*pow(10.0,ii/20.0);
	//	fprintf(ofp,"%d\t %.2E\t %.2E\n",ii,Etest,SpectdNdE(Etest, FluxA)*pow(Etest,2.0));
	fprintf(ofp,"%d\t %.2E\t %.2E\n",ii,Etest,SpectdNdE(Etest, FluxA));
      }
  }
  ...
  // neutrino flux
  sprintf(outputFilename,"/home/ytang/anni_neutrino_%.0f_%.0f.dat",Mcdm, MSx);
  sprintf(outputdNdE,"/home/ytang/dNdE_neutrino_%.0f_%.0f.dat",Mcdm, MSx);
  ...
  { 
	...
	Etest=1.0*pow(10.0,ii/20.0);
	//	fprintf(ofp,"%d\t %.2E\t %.2E\n",ii,Etest,SpectdNdE(Etest, FluxA)*pow(Etest,2.0));
	fprintf(ofpdNE,"%d\t %.2E\t %.2E\n",ii,Etest,SpectdNdE(Etest, SpNe)+SpectdNdE(Etest, SpNm)+SpectdNdE(Etest, SpNl));
      }

     double fi=0.52,dfi=0.05; /* angle of sight and 1/2 of cone angle in [rad] */ 

     gammaFluxTab(fi,dfi, sigmaV, SpNe,  FluxNe);     
     gammaFluxTab(fi,dfi, sigmaV, SpNm,  FluxNm);   
     gammaFluxTab(fi,dfi, sigmaV, SpNl,  FluxNl);   

    for(ii=0;ii<71;ii=ii+1)
      {
	Etest=1.0*pow(10.0,ii/20.0);
	//	fprintf(ofp,"%d\t %.2E\t %.2E\n",ii,Etest,SpectdNdE(Etest, FluxA)*pow(Etest,2.0));
	fprintf(ofp,"%d\t %.2E\t %.2E\n",ii,Etest,SpectdNdE(Etest, FluxNe)+SpectdNdE(Etest, FluxNm)+SpectdNdE(Etest, FluxNl));
      }
  }
  ...
  //
  sprintf(outputFilename,"/home/ytang/anni_positron_%.0f_%.0f.dat",Mcdm, MSx);
  sprintf(outputdNdE,"/home/ytang/dNdE_positron_%.0f_%.0f.dat",Mcdm, MSx);
  ofp = fopen(outputFilename, "w");
  ofpdNE=fopen(outputdNdE, "w");
  ...
  { 
	...
	fprintf(ofp,"%d\t %.2E\t %.2E\n",ii,Etest,SpectdNdE(Etest, FluxE));
      }
  }
  ...
  sprintf(outputFilename,"/home/ytang/anni_antiproton_%.0f_%.0f.dat",Mcdm, MSx);
  sprintf(outputdNdE,"/home/ytang/dNdE_antiproton_%.0f_%.0f.dat",Mcdm, MSx);
  ofp = fopen(outputFilename, "w");
  ofpdNE=fopen(outputdNdE, "w");
  ...
  {      
    for(ii=0;ii<71;ii=ii+1)
      {
	Etest=1.0*pow(10.0,ii/20.0);
	fprintf(ofpdNE,"%d\t %.2E\t %.2E\n",ii,Etest,SpectdNdE(Etest, SpP));
      }
    
    double SMmev=550;// in MeV
    pbarFluxTab(Emin, sigmaV, SpP,  FluxP  ); 

    if(SMmev>0)  solarModulation(SMmev,0.938,FluxP,FluxP);
#ifdef SHOWPLOTS    
    ...
    for(ii=0;ii<71;ii=ii+1)
      {
	Etest=1.0*pow(10.0,ii/20.0);
	fprintf(ofp,"%d\t %.2E\t %.2E\n",ii,Etest,SpectdNdE(Etest, FluxP));
      }
  }
